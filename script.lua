...
--   _G.Config = { UserID="xxx", SignKey="yyy", Note="Pc" }
--
-- UserID   = apiKey dari dashboard (user.apiKey)
-- SignKey  = signing secret untuk HMAC (dari dashboard, atau kosongkan untuk legacy mode)
-- Note     = device label ("Pc", "Android", "Tablet", dll)
-- discord_id gak perlu — otomatis dari database (Discord OAuth).
-- ============================================
local userConfig = _G.Config or {}

local CONFIG = {
    -- Server URL (ganti ke domain VPS)
    API_URL = userConfig.API_URL or "https://1dd9-140-213-216-168.ngrok-free.app/api",

    -- Auth
    USER_ID = userConfig.UserID or nil,
    SIGN_KEY = userConfig.SignKey or nil,  -- HMAC signing secret (opsional, untuk secure mode)
    NOTE = userConfig.Note or "Unknown",

    -- Tracker settings
    -- Base interval 60s + random jitter -10..+10s untuk mencegah heartbeat storm
...
-- FISH IT ADAPTER
-- ============================================
-- Scan results (PlaceId 121864768012064):
--   Net: ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net
--   leaderstats: Caught (Int), Rarest Fish (String)
--   Key remotes: RF/GetPlayerData, RF/SellAllItems, RF/SellItem,
--                RF/Trading/LoadPlayerInventory, RF/Pets/Equip, RF/Pets/Unequip,
--                RF/Pets/OpenEgg, RF/ChargeFishingRod, RF/RequestFishingMinigameStarted
--   Modules: CurrencyUtility, PlayerStatsUtility, InventoryMapping, LevelController
-- ============================================
...
function FishItAdapter:getProgress()
    ...
    if leaderstats then
        ...
        if caught then progress.Caught = caught.Value end
